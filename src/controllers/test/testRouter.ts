...
import { SpotifyArtistsData } from "../spotify/handlers/getSpotifyData/spotifyData.types";
import getSpotifyTopArtistsData from "../spotify/handlers/getSpotifyData/helpers/getSpotifyTopArtistsData";
...
testRouter.get("/resetDB", async (req: Request, res: Response) => {
  ...
  try {
    client.connect(async () => {
      // Connection URL
      const artistData: SpotifyArtistsData[] = await getSpotifyTopArtistsData(
        "BQCF8YI2JReTCRnj0hIUra3Kzul5kxk-aZAWIrDFvsfVhJ43PincN_Nn1XQmZ4mT-tQ8_QUbRJqIcEvrIHvVtyUBoMuomOWWylH73cdmDqIe8Kl2fb1g_fVFVWD6VJd_QTgoQj01WHlfxj6yQyufIsobPgMdR-CSs3nDlu9RI-LCc2sZZYiJANp36V3U"
      );
      ...
      MongoClient.connect(url, function (
        ...
        db.collection("spotify_artists").remove();
        db.collection("spotify_tracks").remove();
      });
      client.db("dummyData").collection("accounts").remove();
      client.db("dummyData").collection("spotify_artists").remove();
...
*/
      ...
      function makeEmail() {
        ...
        var strTmp;
        for (var i = 0; i < 10; i++) {
          strTmp = strValues.charAt(
            Math.round(strValues.length * Math.random())
          );
      ...
      for (i = 0; i < 10; i++) {
        ...
        const passwordHash = bcrypt.hashSync(generatePassword());
        const newAccount = new AccountsModel({
          name: generateName(),
          email: makeEmail(),
          password: passwordHash,
        });
        var randomArtists = getRandom(artistData, 10);
        var artists: string[] = [];
        randomArtists.forEach(async (artist) => {
          const dbArtist = await client
            .db("dummyData")
            .collection("spotify_artists")
            .findOne({
          ...
          if (!dbArtist) {
            ...
            const savedArtist = await client
              .db("dummyData")
              .collection("spotify_artists")
              .insertOne(newArtist);
            newAccount.spotify.artists.push(newArtist._id);
          } else {
            if (!dbArtist.accounts.includes(newAccount._id)) {
              dbArtist.accounts.push(newAccount._id);
            }
            await client
              .db("dummyData")
              .collection("spotify_artists")
              .updateOne(
                { spotifyId: artist.spotifyId },
                { $set: { accounts: dbArtist.accounts } }
              );

            newAccount.spotify.artists.push(dbArtist._id);
          }
        });
